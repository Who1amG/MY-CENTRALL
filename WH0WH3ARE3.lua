...
SCR.DisplayOrder = 1  -- low so game crosshair stays on top
SCR.Parent = LPLR:WaitForChild("PlayerGui")

-- Watch for crosshair/hud ScreenGuis and keep them above our UI
local PG = LPLR:WaitForChild("PlayerGui")
local function RAISE_CROSSHAIR(gui)
    if not gui:IsA("ScreenGui") then return end
    local n = gui.Name:lower()
    if n:find("crosshair") or n:find("hud") or n:find("gun") or n:find("solar") or n:find("weapon") then
        gui.DisplayOrder = math.max(gui.DisplayOrder, SCR.DisplayOrder + 100)
    end
end
for _, g in ipairs(PG:GetChildren()) do RAISE_CROSSHAIR(g) end
PG.ChildAdded:Connect(function(g) task.wait() RAISE_CROSSHAIR(g) end)
...
-- ════════════════════════════════════════════════════════════════════
--  PLAYER VISUALS - CONFIG
-- ════════════════════════════════════════════════════════════════════
local ESP_CONFIG = {
    Enabled = true,
    BoundingBoxes = {Enabled = false, Color = Color3.new(1, 1, 1), Alpha = 1},
    CornerBoxes = {Enabled = false, Color = Color3.new(1, 1, 1), Alpha = 1},
    FilledBoxes = {Enabled = false, Color1 = Color3.fromRGB(119, 120, 255), Color2 = Color3.new(0, 0, 0), Alpha = 0.25},
    Names = {Enabled = true, Color = Color3.new(1, 1, 1), Alpha = 1},
    HealthBars = {Enabled = false, ColorHigh = Color3.new(0, 1, 0), ColorLow = Color3.new(1, 0, 0), Alpha = 1},
    HealthText = {Enabled = false},
    Weapons = {Enabled = false, Color = Color3.new(1, 1, 1), Alpha = 1},
    Distance = {Enabled = false, Color = Color3.new(1, 1, 1), Alpha = 1},
    Chams = {Enabled = false, Color1 = Color3.fromRGB(119, 120, 255), Color2 = Color3.new(0, 0, 0), Alpha = 0.8},
    AnimatedBoxes = false,
    DynamicHealthText = true,
    GradientHealthBar = true,
    ThermalChams = false,
    TextFont = "Plex",
    TextSize = 12,
...
local function CreateColorPicker_PV(parent, initialColor, callback)
    ...
    local corner = Instance.new("UICorner", colorButton)
    corner.CornerRadius = UDim.new(0, 4)
    
    local stroke = Instance.new("UIStroke", colorButton)
    stroke.Color = Color3.new(0.5, 0.5, 0.5)
    stroke.Thickness = 1
    
    colorButton.MouseButton1Click:Connect(function()
        if callback then callback(initialColor) end
    end)
    
    return colorButton
end

local function CreateToggle_PV(parent, label, initialState, callback)
    ...
    toggle.BackgroundColor3 = initialState and Color3.new(0, 0.8, 0.2) or Color3.new(0.3, 0.3, 0.3)
    ...
    toggle.MouseButton1Click:Connect(function()
        state = not state
        toggle.BackgroundColor3 = state and Color3.new(0, 0.8, 0.2) or Color3.new(0.3, 0.3, 0.3)
...
local VIS_CONTAINER = Instance.new("Frame", P_VIS)
VIS_CONTAINER.Size = UDim2.new(1, -10, 0, 0)
VIS_CONTAINER.AutomaticSize = Enum.AutomaticSize.Y
VIS_CONTAINER.BackgroundTransparency = 1
VIS_CONTAINER.BorderSizePixel = 0

local VIS_LAYOUT = Instance.new("UIListLayout", VIS_CONTAINER)
VIS_LAYOUT.SortOrder = Enum.SortOrder.LayoutOrder
VIS_LAYOUT.Padding = UDim.new(0, 8)

local TITLE1 = Instance.new("TextLabel", VIS_CONTAINER)
TITLE1.Size = UDim2.new(1, 0, 0, 25)
TITLE1.BackgroundTransparency = 1
TITLE1.Text = "👁️ Player Visuals"
TITLE1.TextColor3 = Color3.new(0.2, 0.7, 1)
TITLE1.TextSize = 16
TITLE1.Font = Enum.Font.GothamBold
TITLE1.TextXAlignment = Enum.TextXAlignment.Left
TITLE1.LayoutOrder = 1

local SEP1 = Instance.new("Frame", VIS_CONTAINER)
SEP1.Size = UDim2.new(1, 0, 0, 1)
SEP1.BackgroundColor3 = Color3.new(0.3, 0.3, 0.3)
SEP1.BorderSizePixel = 0
SEP1.LayoutOrder = 2

local order = 3

local _, _, getEnabledState = CreateToggle_PV(VIS_CONTAINER, "Enabled", ESP_CONFIG.Enabled, function(state)
    ESP_CONFIG.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getBBState = CreateToggle_PV(VIS_CONTAINER, "Bounding Boxes", ESP_CONFIG.BoundingBoxes.Enabled, function(state)
    ESP_CONFIG.BoundingBoxes.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local bbColorContainer = Instance.new("Frame", VIS_CONTAINER)
bbColorContainer.Size = UDim2.new(1, 0, 0, 35)
bbColorContainer.BackgroundTransparency = 1
bbColorContainer.BorderSizePixel = 0
bbColorContainer.LayoutOrder = order; order += 1

local bbColorLbl = Instance.new("TextLabel", bbColorContainer)
bbColorLbl.Size = UDim2.new(0.7, 0, 1, 0)
bbColorLbl.Position = UDim2.new(0, 20, 0, 0)
bbColorLbl.BackgroundTransparency = 1
bbColorLbl.Text = "  Color"
bbColorLbl.TextColor3 = Color3.new(0.8, 0.8, 0.8)
bbColorLbl.TextSize = 12
bbColorLbl.Font = Enum.Font.Gotham
bbColorLbl.TextXAlignment = Enum.TextXAlignment.Left

CreateColorPicker_PV(bbColorContainer, ESP_CONFIG.BoundingBoxes.Color, function(color)
    ESP_CONFIG.BoundingBoxes.Color = color
end)

local _, _, getCBState = CreateToggle_PV(VIS_CONTAINER, "Corner Boxes", ESP_CONFIG.CornerBoxes.Enabled, function(state)
    ESP_CONFIG.CornerBoxes.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local cbColorContainer = Instance.new("Frame", VIS_CONTAINER)
cbColorContainer.Size = UDim2.new(1, 0, 0, 35)
cbColorContainer.BackgroundTransparency = 1
cbColorContainer.BorderSizePixel = 0
cbColorContainer.LayoutOrder = order; order += 1

local cbColorLbl = Instance.new("TextLabel", cbColorContainer)
cbColorLbl.Size = UDim2.new(0.7, 0, 1, 0)
cbColorLbl.Position = UDim2.new(0, 20, 0, 0)
cbColorLbl.BackgroundTransparency = 1
cbColorLbl.Text = "  Color"
cbColorLbl.TextColor3 = Color3.new(0.8, 0.8, 0.8)
cbColorLbl.TextSize = 12
cbColorLbl.Font = Enum.Font.Gotham
cbColorLbl.TextXAlignment = Enum.TextXAlignment.Left

CreateColorPicker_PV(cbColorContainer, ESP_CONFIG.CornerBoxes.Color, function(color)
    ESP_CONFIG.CornerBoxes.Color = color
end)

local _, _, getFBState = CreateToggle_PV(VIS_CONTAINER, "Filled Boxes", ESP_CONFIG.FilledBoxes.Enabled, function(state)
    ESP_CONFIG.FilledBoxes.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getNamesState = CreateToggle_PV(VIS_CONTAINER, "Names", ESP_CONFIG.Names.Enabled, function(state)
    ESP_CONFIG.Names.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local namesColorContainer = Instance.new("Frame", VIS_CONTAINER)
namesColorContainer.Size = UDim2.new(1, 0, 0, 35)
namesColorContainer.BackgroundTransparency = 1
namesColorContainer.BorderSizePixel = 0
namesColorContainer.LayoutOrder = order; order += 1

local namesColorLbl = Instance.new("TextLabel", namesColorContainer)
namesColorLbl.Size = UDim2.new(0.7, 0, 1, 0)
namesColorLbl.Position = UDim2.new(0, 20, 0, 0)
namesColorLbl.BackgroundTransparency = 1
namesColorLbl.Text = "  Color"
namesColorLbl.TextColor3 = Color3.new(0.8, 0.8, 0.8)
namesColorLbl.TextSize = 12
namesColorLbl.Font = Enum.Font.Gotham
namesColorLbl.TextXAlignment = Enum.TextXAlignment.Left

CreateColorPicker_PV(namesColorContainer, ESP_CONFIG.Names.Color, function(color)
    ESP_CONFIG.Names.Color = color
end)

local _, _, getHBState = CreateToggle_PV(VIS_CONTAINER, "Health Bars", ESP_CONFIG.HealthBars.Enabled, function(state)
    ESP_CONFIG.HealthBars.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getHTState = CreateToggle_PV(VIS_CONTAINER, "Health Text", ESP_CONFIG.HealthText.Enabled, function(state)
    ESP_CONFIG.HealthText.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getWeapState = CreateToggle_PV(VIS_CONTAINER, "Weapons", ESP_CONFIG.Weapons.Enabled, function(state)
    ESP_CONFIG.Weapons.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local weapColorContainer = Instance.new("Frame", VIS_CONTAINER)
weapColorContainer.Size = UDim2.new(1, 0, 0, 35)
weapColorContainer.BackgroundTransparency = 1
weapColorContainer.BorderSizePixel = 0
weapColorContainer.LayoutOrder = order; order += 1

local weapColorLbl = Instance.new("TextLabel", weapColorContainer)
weapColorLbl.Size = UDim2.new(0.7, 0, 1, 0)
weapColorLbl.Position = UDim2.new(0, 20, 0, 0)
weapColorLbl.BackgroundTransparency = 1
weapColorLbl.Text = "  Color"
weapColorLbl.TextColor3 = Color3.new(0.8, 0.8, 0.8)
weapColorLbl.TextSize = 12
weapColorLbl.Font = Enum.Font.Gotham
weapColorLbl.TextXAlignment = Enum.TextXAlignment.Left

CreateColorPicker_PV(weapColorContainer, ESP_CONFIG.Weapons.Color, function(color)
    ESP_CONFIG.Weapons.Color = color
end)

local _, _, getDistState = CreateToggle_PV(VIS_CONTAINER, "Distance", ESP_CONFIG.Distance.Enabled, function(state)
    ESP_CONFIG.Distance.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local distColorContainer = Instance.new("Frame", VIS_CONTAINER)
distColorContainer.Size = UDim2.new(1, 0, 0, 35)
distColorContainer.BackgroundTransparency = 1
distColorContainer.BorderSizePixel = 0
distColorContainer.LayoutOrder = order; order += 1

local distColorLbl = Instance.new("TextLabel", distColorContainer)
distColorLbl.Size = UDim2.new(0.7, 0, 1, 0)
distColorLbl.Position = UDim2.new(0, 20, 0, 0)
distColorLbl.BackgroundTransparency = 1
distColorLbl.Text = "  Color"
distColorLbl.TextColor3 = Color3.new(0.8, 0.8, 0.8)
distColorLbl.TextSize = 12
distColorLbl.Font = Enum.Font.Gotham
distColorLbl.TextXAlignment = Enum.TextXAlignment.Left

CreateColorPicker_PV(distColorContainer, ESP_CONFIG.Distance.Color, function(color)
    ESP_CONFIG.Distance.Color = color
end)

local _, _, getChamsState = CreateToggle_PV(VIS_CONTAINER, "Chams", ESP_CONFIG.Chams.Enabled, function(state)
    ESP_CONFIG.Chams.Enabled = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local SEP2 = Instance.new("Frame", VIS_CONTAINER)
SEP2.Size = UDim2.new(1, 0, 0, 1)
SEP2.BackgroundColor3 = Color3.new(0.3, 0.3, 0.3)
SEP2.BorderSizePixel = 0
SEP2.LayoutOrder = order; order += 1

local TITLE2 = Instance.new("TextLabel", VIS_CONTAINER)
TITLE2.Size = UDim2.new(1, 0, 0, 25)
TITLE2.BackgroundTransparency = 1
TITLE2.Text = "⚙️ Visual Settings"
TITLE2.TextColor3 = Color3.new(0.2, 0.7, 1)
TITLE2.TextSize = 16
TITLE2.Font = Enum.Font.GothamBold
TITLE2.TextXAlignment = Enum.TextXAlignment.Left
TITLE2.LayoutOrder = order; order += 1

local _, _, getAnimState = CreateToggle_PV(VIS_CONTAINER, "Animated Boxes", ESP_CONFIG.AnimatedBoxes, function(state)
    ESP_CONFIG.AnimatedBoxes = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getDHTState = CreateToggle_PV(VIS_CONTAINER, "Dynamic Health Text", ESP_CONFIG.DynamicHealthText, function(state)
    ESP_CONFIG.DynamicHealthText = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getGHBState = CreateToggle_PV(VIS_CONTAINER, "Gradient Health Bar", ESP_CONFIG.GradientHealthBar, function(state)
    ESP_CONFIG.GradientHealthBar = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1

local _, _, getThermalState = CreateToggle_PV(VIS_CONTAINER, "Thermal Chams", ESP_CONFIG.ThermalChams, function(state)
    ESP_CONFIG.ThermalChams = state
end)
VIS_CONTAINER:FindFirstChildWhichIsA("Frame", true).LayoutOrder = order; order += 1
...
do
    ...
    -- ── VIM para pressE ─────────────────────────────────────
    local VIM_CF = game:GetService("VirtualInputManager")
    local function CF_PRESS_E()
        VIM_CF:SendKeyEvent(true,  "E", false, game)
        task.wait(1.6)
        VIM_CF:SendKeyEvent(false, "E", false, game)
    end
...
local VIM_WH = game:GetService("VirtualInputManager")
...
local function WH_HOLD_PROMPT(prompt)
    local key  = prompt.KeyboardKeyCode
    local hold = prompt.HoldDuration
    VIM_WH:SendKeyEvent(true,  key, false, game)
    task.wait(hold + 0.15)
    VIM_WH:SendKeyEvent(false, key, false, game)
end
...
local function WH_DO_ONE()
    ...
    -- Hold the prompt (VirtualInputManager sends raw keys)
    WH_HOLD_PROMPT(pp)
    task.wait(0.2)
...
local VIM_BF = game:GetService("VirtualInputManager")
...
local function BF_HOLD_PROMPT(prompt)
    if not prompt then return end
    local key  = prompt.KeyboardKeyCode or Enum.KeyCode.E
    local hold = prompt.HoldDuration or 0
    VIM_BF:SendKeyEvent(true, key, false, game)
    task.wait(hold + 0.15)
    VIM_BF:SendKeyEvent(false, key, false, game)
end
...
local MOV_WRAP  -- upvalue so CAR block can access it
do
...
do
    ...
    local COL_R = Instance.new("Frame", MOV_WRAP)
    ...
    D_TOOLS.SCR.CanvasSize = UDim2.new(0, 0, 0, D_TOOLS.LAY.AbsoluteContentSize.Y)
